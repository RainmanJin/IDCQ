...
public class JinHD extends Dialog {
	...
	public JinHD(Shell parent, int style) {
		super(parent, style);
	}

	public JinHD(Shell parent) {
	...
	protected void createContents() {
		...
		shell.setImage(SWTResourceManager.getImage(JinHD.class, "/picture/6.jpg"));
		shell.setSize(743, 528);
		...
		button.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(final SelectionEvent e) {
				...
				new JinHD_Add(Display.getCurrent().getActiveShell()).open(name,JinHD.this);
				int sl = 0;
				double menoy = 0.0;
		...
		button.setText("添加");
		button.setBounds(553, 447, 48, 26);
		...
		button_1.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(final SelectionEvent e) {
				...
				TbRukuMain ruMain = new TbRukuMain(
						text.getText().trim(), 
						text_5.getText().trim(), 
						text_7.getText().trim(), 
						text_8.getText().trim(),
				...
				if(rs) {
					MessageDialog.openInformation(shell, "提示", "入库成功!");
					table.removeAll();
					text_3.setText("");
					text_8.setText("");
				}
		...
		button_1.setText("入库");
		button_1.setBounds(650, 447, 48, 26);
	...
	public void initTime() {
		...
		String maxId = Dao.getRuKuMainMaxId(de);
		text.setText(maxId);
	}
}
